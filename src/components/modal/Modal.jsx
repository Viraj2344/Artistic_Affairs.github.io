...
export default function Modal({
  ...
  const handleOrderNow = () => {
    if (isFormComplete) {
      ...
      // Handle incomplete form submission (show an alert, error message, etc.)
      console.error('Please fill out all details before placing the order.');
    }
  };
                        ...
                        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                          ...
                          <button
                            ...
                            className={`focus:outline-none w-full text-white ${
                              isFormComplete ? 'bg-violet-600 hover:bg-violet-800' : 'bg-gray-400 cursor-not-allowed'
                            } outline-0 font-medium rounded-lg text-sm px-5 py-2.5`}
                          >
                            Order Now
                          </button>
                        </div>
